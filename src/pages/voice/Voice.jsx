...
const renderGroup = (content) => (
  ...
  const { summary, file_path } = location.state || {}; 
  
  const handlePlayButtonClick = () => {
    window.location.href = file_path; // Redirect to the file_path link
  };
            ...
            <div className="Group490" style={{ width: 292, height: 199, top: 30, position: 'relative', left: 'calc(50% - 146px)' }}>
              <div style={{width: 136, height: 34.08, left: 48, top: 150, position: 'absolute'}}>
                <div className="ButtonsCtaSecondary" style={{width: 136, height: 34.08, left: 30, top: -1, position: 'absolute', background: '#F3AF31', boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.25)', borderRadius: 8, backdropFilter: 'blur(4px)'}}>
                  <div className="ButtonsCtaSecondary" style={{width: 136, height: 34.08, left: 0, top: 0, position: 'absolute', borderRadius: 25}} onClick={handlePlayButtonClick}>
                  </div>
                </div>
                <div style={{width: 136, height: 7.26, left: 30, top: 11, position: 'absolute', textAlign: 'center', color: 'white', fontSize: 16, fontFamily: 'Noto Sans KR', fontWeight: '700', wordWrap: 'break-word'}} onClick={handlePlayButtonClick} >재생하기</div>
              </div>
